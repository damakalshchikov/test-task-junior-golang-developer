...
- **Unit** (`internal/models`, `internal/api`): хендлеры тестируются через роутер на реализации интерфейса `SubscriptionStorage`.
- **Integration** (`internal/storage/postgres`): через [testcontainers](https://golang.testcontainers.org/) поднимает временный контейнер `postgres:16-alpine`, накатывает миграции и удаляет его после прогона.
